...
static void esp8266_uart_callback(void)
{
	...
	/* Read UART character and store into buffer */
	esp8266_uart_read(&recv_ch, 1);
	esp8266_data_buffer[esp8266_data_indx++] = recv_ch;

	/* Monitor if expected response to AT command is received */
...
#if 0
	// TODO: Just publishing only on 'temperature' topic.
	// Not handling events from other topics
