...
esp8266_mqtt_publish_config temperature_publish = {
	.mqtt_data = NULL,
	.mqtt_data_len = 0,
	.packet_id = 0,
	.qos = 0,
	.retained_flag = 0,
	.topic = (uint8_t *)"event/temperature"
};

void esp8266_parse_mqtt_msgs(void)
{
	// TODO - Publishing data only
...
esp8266_wifi_status esp8266_mqt_msg_publish(void)
{
	...
	// TODO: Publishing temperature only
	publish_topic = TEMPERATURE;
	delay_ms(1000);
	...
	case TEMPERATURE:
		...
		/* Serialize the mqtt packet */
		topicString.cstring = (char *)temperature_publish.topic;
		temperature_publish.mqtt_data = (uint8_t *)temperature;
		temperature_publish.mqtt_data_len = strlen(temperature);

		data_len = MQTTSerialize_publish((unsigned char *)mqtt_msg_buffer,
						 500, 0u, temperature_publish.qos, 0u,
						 temperature_publish.packet_id, topicString,
						 (unsigned char *)temperature_publish.mqtt_data,
						 temperature_publish.mqtt_data_len);
